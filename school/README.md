...
Le fichier de fixtures (`test_data_fixtures.json`) et le script `populate_fake_data.py` génèrent par défaut ces comptes :

| Rôle | Identifiant de connexion (Username / Email) | Mot de passe | Accès Principal |
|------|---------------------------------------------|--------------|-----------------|
| **Administrateur** | *(Votre compte superuser)*| `(À créer)`  | Tableau de bord complet, CRUD Universel, Sécurité |
| **Professeur**     | `ayoub` *(ou ayoub@example.com)* | `password123`| Vue de ses matières assignées, Saisie sécurisée des notes |
| **Étudiant**       | `youssef` *(ou youssef@example.com)*| `password123`| Consultation de l'historique d'apprentissage, Emploi du temps, Notes |
*(Note : Plusieurs autres élèves de test tels que fatima, omar, khadija et amine existent également avec le mot de passe `password123`).*
